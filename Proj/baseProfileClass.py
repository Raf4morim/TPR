...
pattern1 = 'Features/brwsg2Wind_features*'
# pattern2 = 'Features/attackSeqWind_features*'
# pattern3 = 'attackSmartWind_features*'

file_list1 = glob.glob(pattern1)
# file_list2 = glob.glob(pattern2)

if not file_list1:
    raise ValueError(f"No files found for pattern {pattern1}")
# if not file_list2:
#     raise ValueError(f"No files found for pattern {pattern2}")
...
for idx, features in enumerate(all_features1):
    print(f"Shape of array {idx}: {features.shape}")
    print(f'FEEEEEEEEEEEEEEEATURES BRWSG', features)

# print("FEAATURESSSSSSSSSSS MININGGG",features_mining)
...
features = np.vstack((features_browsing, features_mining))
oClass = np.vstack((oClass_browsing, oClass_mining))
...
i3Ctrain = np.vstack((trainFeatures_browsing, trainFeatures_mining))
o3trainClass = np.vstack((oClass_browsing[:pB], oClass_mining[:pM]))

testFeatures_browsing = features_browsing[pB:, :]
testFeatures_mining = features_mining[pM:, :]

i3Atest = np.vstack((testFeatures_browsing, testFeatures_mining))
o3testClass = np.vstack((oClass_browsing[pB:], oClass_mining[pM:]))

i3train=np.vstack((trainFeatures_browsing,trainFeatures_mining))
i3Ctest=np.vstack((testFeatures_browsing,testFeatures_mining))
