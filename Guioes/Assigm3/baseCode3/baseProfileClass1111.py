...
def plotClusteredFeatures(features, labels, f1index=0, f2index=1):
    nObs, nFea = features.shape
# Mapear os rótulos dos clusters para as cores originais
    colors = ['b', 'g', 'r']  # Cores originais: azul para Browsing, verde para YouTube e vermelho para Mining
    cluster_colors = [colors[int(label)] for label in labels]

    # Plotar pontos de dados com cores diferentes para cada cluster
    for i in range(nObs):
        cluster_label = labels[i]
        plt.scatter(features[i, f1index], features[i, f2index], color=cluster_colors[cluster_label])
...
plt.figure(3)
plotFeatures(features,oClass,0,7) #? comparar media upload com mediana download?
...
# 4 clusteres para separar o comportamento do yt
for i in range(len(labels)):
    print('Obs: {:2} ({}): K-Means Cluster Label: -> {}'.format(i,Classes[o3testClass[i][0]],labels[i]))

    print("AAAAAAAAAAHHHHH"+Classes[o3testClass[i][0]])
    
# Suponha que 'i3Ctrain' contenha seus dados de treinamento e 'labels' contém os rótulos de clusters obtidos do K-Means
plt.figure(7)
plotClusteredFeatures(i3Ctrain, labels)
## -- 5 -- ##
